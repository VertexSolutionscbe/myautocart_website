...
if(isset($_POST['form1'])) {
	...
	// } else {
		...
		// $statement = $pdo->prepare("SELECT * FROM tbl_make WHERE model=? and model!=?");
	...
	// if($valid == 1) {
		...
		// if($previous_photo != '' && $path != '') {
	        ...
	        $statement = $pdo->prepare("UPDATE tbl_make SET make=?, category_name=? WHERE id=?");
	    	$statement->execute(array($_POST['make'],$_POST['category_id'],$_REQUEST['id']));
	    //}

	    $success_message = 'Make is updated successfully!';
	//}
}
?>
...
?>
    ...
    <?php
$statement = $pdo->prepare("SELECT * FROM tbl_make WHERE id=?");
$statement->execute(array($_REQUEST['id']));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
	$make   = $row['make'];	
