...
if(isset($_POST['form1'])) {
	...
	if($path!='') {
	    ...
	    if($previous_photo == '' && $path == '') {
	    	$statement = $pdo->prepare("UPDATE tbl_spares SET vendor_id=?,category_id=?,date=?,spare_name=?,spare_amount=? WHERE id=?");
	    	$statement->execute(array($_POST['vendor_id'],$_POST['category_id'],$_POST['date'],$_POST['spare_name'],$_POST['spare_amount'],$_REQUEST['id']));
	  
	    	$statement = $pdo->prepare("UPDATE tbl_products SET category_id=?,product_date=?,product_title=?,product_amount=?,product_status=? WHERE product_id=?");
	    	$statement->execute(array($_POST['category_id'],$_POST['date'],$_POST['spare_name'],$_POST['spare_amount'],'Active',$_REQUEST['pid']));
	  }
	    ...
	    if($previous_photo == '' && $path != '') {
	    	...
	    	$final_name = 'Spare-'.$_REQUEST['id'].'.'.$ext;
            move_uploaded_file( $path_tmp, '../dist/uploads/'.$final_name );

	    	$statement = $pdo->prepare("UPDATE tbl_spares SET photo=?,vendor_id=?,category_id=?,date=?,spare_name=?,spare_amount=? WHERE id=?");
	    	$statement->execute(array($final_name,$_POST['vendor_id'],$_POST['category_id'],$_POST['date'],$_POST['spare_name'],$_POST['spare_amount'],$_REQUEST['id']));	
	    	...
	    	$statement->execute(array($final_name,$_POST['category_id'],$_POST['date'],$_POST['spare_name'],$_POST['spare_amount'],$_REQUEST['pid']));
			}
	    ...
	    $success_message = 'Spare updated successfully!';
	}
...
?>
      ...
      <div class="main-content">
                ...
                <div class="card">
                  ...
                  <div class="card-body">
                    <div class="form-group row mb-4">
                      ...
                      <div class="col-sm-12 col-md-7">
                        <input type="text" name="spare_name" id="spare_name" value="<?php //echo $spare_name; ?>" class="form-control">
                      </div>
                    </div>
					<div class="form-group row mb-4">
                      <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Spare Amount</label>
                      <div class="col-sm-12 col-md-7">
                        <input type="text" name="spare_amount" id="spare_amount" value="<?php //echo $spare_amount; ?>" class="form-control">
                      </div>
                    </div>
		...
		<!-- Form Start -->			
		  <iframe src="http://13.235.247.42/SparesEdit?id=<?php echo $_REQUEST['id']; ?>&pid=<?php echo $_REQUEST['pid']; ?>" width="100%" height="2000px" style="border:none;"> </iframe>
		<!-- Form Start End -->  
          </div>
        </section>
      </div>
