...
if(isset($_POST['form1'])) {
	...
	// if($valid == 1) {
		...
		// if($previous_photo != '' && $path != '') {
	    	...
	    	$statement = $pdo->prepare("UPDATE tbl_services SET service_name=?,services_include=?,gst_per=? WHERE id=?");
	    	$statement->execute(array($_POST['service_name'],$_POST['services_include'],$_POST['gst_per'],$_REQUEST['id']));
	    //}
...
foreach ($result as $row) {
	...
	$services_include = $row['services_include'];
	$gst_per   = $row['gst_per'];
   }
?>
                  ...
                  <div class="card-body">
                   ...
                   <div class="form-group row mb-4">
                    ...
                    <div class="form-group row mb-4">
                      ...
                      <div class="col-sm-12 col-md-7">
                        <textarea  class="form-control summernote-simple" name="services_include" id="services_include"><?php echo $services_include; ?></textarea>
                      </div>
                    </div>						
				   ...
				   <div class="form-group row mb-4">
                      ...
                      <div class="col-sm-12 col-md-7">
                        <select class="form-control selectric" name="gst_per" id="gst_per">
						    <option value="">Select the GST</option>
							<?php
						   
						      $statement = $pdo->prepare("SELECT * FROM tbl_gst order by  id asc");
						      $statement->execute();
						      $result = $statement->fetchAll(PDO::FETCH_ASSOC);
						      foreach ($result as $row) {
						    ?>						  
                          <option value="<?php echo $row['gst']; ?>" <?php if($row['gst']==$gst_per){echo 'selected';} ?>><?php echo $row['gst']; ?></option>                          
						  <?php } ?>
                        </select>
                      </div>
                    </div>					
