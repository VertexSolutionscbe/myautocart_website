...
if(isset($_POST['form1'])) {
	$valid = 1;
	 ...
	 if(empty($_POST['color'])) {
		$valid = 0;
		$error_message .= 'color title can not be empty<br>';
	} else {
		// Duplicate Category checking
    	// current Product title that is in the database
    	$statement = $pdo->prepare("SELECT * FROM tbl_color WHERE id=?");
		$statement->execute(array($_REQUEST['id']));
		$result = $statement->fetchAll(PDO::FETCH_ASSOC);
		foreach($result as $row) {
			$current_product_title = $row['color'];
		}

		$statement = $pdo->prepare("SELECT * FROM tbl_color WHERE color=? and color!=?");
    	$statement->execute(array($_POST['color'],$current_product_title));
	...
	// if($valid == 1) {
		...
		// if($previous_photo != '' && $path != '') {
	    	...
	    	$statement = $pdo->prepare("UPDATE tbl_color SET color=? WHERE id=?");
                ...
                <div class="card">
                  ...
                  <div class="card-body">
                    ...
                    <div class="form-group row mb-4">
                      <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Color Name</label>
