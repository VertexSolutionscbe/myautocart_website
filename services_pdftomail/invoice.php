...
<td align="left" width="345"><B><u> Customer details </u><B><BR/>'.$Fcn['f_name'].'<BR/>'.$Fcn['billing_address'].'<BR/>'.$Fcn['mobile'].'<BR/>'.$Fcn['email'].'<BR/>'.$EFrd['vehicle_no'].'<BR/>'.$FEmo['model'].'</td>
<td>Invoice No<br/>Order No<br/>Date<br/>Time</td>
<td>: <br/>:<br/>:<br/>:</td>
<td>'.$EFrd['inv_no'].'<br/>'.$EFrd['service_order_no'].'<br/>'.date("d-m-Y", strtotime($EFrd['appointment_date'])).'<br/>'.$EFrd['appointment_time'].'</td>
</tr>
</table>
</td>
</tr>';

$output.='<tr><td align="left"  height="50px"><b>Service Details :</b></td></tr>
<tr>
<td>
...
<td bgcolor="#000000" align="center"><FONT COLOR="FFFFFF">Service Name</FONT></td>
    ...
    $i=0;
        ...
        //$FEli=mysqli_fetch_array($Eli);
            ...
            $FEcus=mysqli_fetch_array($Ecus);
			    ...
			    $pro="SELECT * FROM `tbl_services` WHERE id='".$FEli['vendor_service_id']."'";
                ...
                $FEpro=mysqli_fetch_array($Epro);
                   ...
                   $FEcat=mysqli_fetch_array($Ecat);
				     ...
				     $service_name         = $FEpro['service_name'];
					 $amount	           = $FEli['amount'];					 
					 $rate	           = $FEli['taxable_amount'];					 
...
<td bgcolor="#FFFFFF" align="center">'.$FEli['cgst'].'</td>
<td bgcolor="#FFFFFF" align="center">'.$FEli['sgst'].'</td>
...
<html>
	...
	<body>
		<br/>
		<div class="container">
			<h3 align="center">Service Invoice</h3>
			<br/>
			<form method="post">
