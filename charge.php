...
if($_POST!='')
{
	// print_r($_POST);
	// echo "</br>";
	// echo "Transaction Success!".$_POST['razorpay_payment_id'];
	$rid=$_POST['razorpay_payment_id'];	
	// Array ( [hidden] => Hidden Element [razorpay_payment_id] => pay_ERBTYKoccb5Do5 )
	// Transaction Success!pay_ERBTYKoccb5Do5
...
$sc="select f_name,email,mobile from tbl_customer where customer_id='".$CustomerId."'";
...
/*Start - SMS to Customer*/
$ch = curl_init();
$user="nazeer.sheik@vertexsolution.co.in:vertex";
$receipientno=$FEsc['mobile'];
$senderID="TEST SMS";
$msgtxt="Your Order on MyAutoCart is confirmed.Please note your Order no ".$FEng['OrderNo'] ;
...
//$rno="1234567";
$sv="select vendor_name,email,contact_no from tbl_vendor where vendor_id='".$vendorId."'";
...
$ch = curl_init();
$user="nazeer.sheik@vertexsolution.co.in:vertex";
$receipientno=$FEsv['contact_no'];
$senderID="TEST SMS";
$msgtxt="Hi,New Order received from MyAutocart.Please note Order no is ".$FEng['OrderNo'] ;
curl_setopt($ch,CURLOPT_URL,  "http://api.mVaayoo.com/mvaayooapi/MessageCompose");
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_POST, 1);
curl_setopt($ch, CURLOPT_POSTFIELDS, "user=$user&senderID=$senderID&receipientno=$receipientno&msgtxt=$msgtxt");
...
{
	header("location:shopping-cart.php?m=2");
}
